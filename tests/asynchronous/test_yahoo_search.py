...
class TestAsyncYahooSearchApi(unittest.TestCase):

		def setUp(self):
				AsyncYahooSearch.SERP_API_KEY = os.getenv("API_KEY", "demo")

		@unittest.skipIf((os.getenv("API_KEY") == None), "no api_key provided")
		async def test_get_json(self):
				search = AsyncYahooSearch({"p": "Coffee"})
		...
		async def test_paginate(self):
				page_size = 7
				search = AsyncYahooSearch({"p": "Coffee", "b": page_size, "pz": page_size})
