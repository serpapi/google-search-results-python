...
class TestAsyncExample(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_json(self):
        search = AsyncGoogleSearch({"q": "Coffee", "engine": "google_scholar"})
    ...
    @unittest.skipIf((os.getenv("DEBUGAPI_KEY") is None), "no api_key provided")
    async def test_search_google_images(self):
        search = AsyncGoogleSearch({"q": "coffe", "tbm": "isch"})
        for image_result in (await search.get_json())["images_results"]:
            try:
    ...
    async def test_async(self):
        ...
        # Serp API search
        search = AsyncGoogleSearch({"location": "Austin,Texas", "async": True})
        ...
        search = AsyncGoogleSearch({"async": True})
        while not search_queue.empty():
            data = await search_queue.get()

            search_id = data["search_metadata"]["id"]

            # retrieve search from the archive - blocker
            print(search_id + ": get search from archive")
            search_archived = await search.get_search_archive(search_id)
            print(
                search_id + ": status = " + search_archived["search_metadata"]["status"]
            )
    ...
    @unittest.skipIf((os.getenv("DEBUGAPI_KEY") is None), "no api_key provided")
    async def test_search_google_news(self):
    ...
    @unittest.skipIf((os.getenv("DEBUGAPI_KEY") is None), "no api_key provided")
    async def test_search_google_shopping(self):
        search = AsyncGoogleSearch(
            {
                "q": "coffe",  # search search
                "tbm": "shop",  # news
        ...
        if "shopping_results" in data:
            for shopping_result in data["shopping_results"]:
                print(
                    str(shopping_result["position"]) + " - " + shopping_result["title"]
                )
        else:
            print(
    ...
    async def test_search_by_location(self):
        for city in ["new york", "paris", "berlin"]:
            location = (await AsyncGoogleSearch({}).get_location(city, 1))[0][
                "canonical_name"
            ]
            search = AsyncGoogleSearch(
                {
