...
class TestAsyncSearchApi(unittest.TestCase):
		...
		async def test_paginate(self):
				page_size = 20
				search = AsyncGoogleSearch({"q": "Coffee", "location": "Austin,Texas", "start": 10, "num": page_size})
		...
		@unittest.skipIf((os.getenv("API_KEY") == None), "no api_key provided")
		async def test_get_json_2(self):
		...
		async def test_get_dictionary(self):
				search = AsyncGoogleSearch({"q": "Coffee", "location": "Austin,Texas"})
				data = await search.get_dictionary()
				self.assertIsNotNone(data.get('local_results'))

		@unittest.skipIf((os.getenv("API_KEY") == None), "no api_key provided")
		async def test_get_html(self):
				search = AsyncGoogleSearch({"q": "Coffee", "location": "Austin,Texas"})
				data = await search.get_html()
		...
		async def test_get_response(self):
				search = AsyncGoogleSearch({"q": "Coffee", "location": "Austin,Texas"})
				response = await search.get_response()
